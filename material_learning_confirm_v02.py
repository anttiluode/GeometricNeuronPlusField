"""Evaluate MATERIAL_LEARNING_CONFIRM_PREREG_V02.

Only L0 differs from v0.1: derivative checks are scale aware so numerical
finite-difference residue at a true zero derivative is audited in absolute
rather than relative units.  L1-L7 are intentionally unchanged.
"""
...
def gradient_audit(rows):
    nonzero=[]
    nearzero=[]
    all_abs=[]
    for r in rows:
        seed=int(r['seed'])
        for q in r['gradient_check']['rows']:
            an=float(q['analytic']);fd=float(q['finite_difference'])
            scale=abs(an)+abs(fd);ae=abs(an-fd);all_abs.append(ae)
            rec=dict(seed=seed,analytic=an,finite_difference=fd,scale=scale,absolute_error=ae)
            if scale>=NONZERO_SCALE:
                rec['relative_error']=ae/(scale+1e-30);nonzero.append(rec)
            else:
                nearzero.append(rec)
    max_rel=max([q['relative_error'] for q in nonzero],default=0.0)
    max_zero_abs=max([q['absolute_error'] for q in nearzero],default=0.0)
    return dict(
        pass_=bool(max_rel<REL_TOL and max_zero_abs<ZERO_ABS_TOL),
        nonzero_samples=len(nonzero),nearzero_samples=len(nearzero),
        max_nonzero_relative_error=float(max_rel),
        max_nearzero_absolute_error=float(max_zero_abs),
        max_absolute_error_all=float(max(all_abs,default=0.0)),
        nonzero_threshold=float(NONZERO_SCALE),relative_tolerance=float(REL_TOL),
        nearzero_absolute_tolerance=float(ZERO_ABS_TOL),
        worst_nonzero=max(nonzero,key=lambda q:q['relative_error']) if nonzero else None,
        worst_nearzero=max(nearzero,key=lambda q:q['absolute_error']) if nearzero else None,
    )
...
def evaluate(p):
    ...
    crit={}
    audit=gradient_audit(rows)
    crit['L0_zero_safe_gradient_audit']=audit
    ...
    return dict(
        prereg='MATERIAL_LEARNING_CONFIRM_PREREG_V02.md',bodies=n,criteria=crit,
        passed=passed,total=len(crit),all_pass=bool(passed==len(crit)),
        pooled=dict(mean_gain_uniform=float(gu.mean()),mean_gain_shuffle=float(gs.mean()),
                    mean_gain_hand=float(gh.mean()),mean_spearman=float(rho.mean()),
                    median_amp_ratio=float(np.median(ar)),mean_phase_rms_gain=float(phasegain.mean())))


def main():
    ap=argparse.ArgumentParser();ap.add_argument('--receipt',required=True);ap.add_argument('--out',required=True);a=ap.parse_args()
    p=json.loads(Path(a.receipt).read_text(encoding='utf-8'));s=evaluate(p)
    out=Path(a.out);out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(s,indent=2),encoding='utf-8')
    print(json.dumps(s,indent=2))
    if not s['all_pass']:raise SystemExit('Held-out material-learning v0.2 failed one or more frozen criteria')
