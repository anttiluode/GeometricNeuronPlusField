...
def free_feature_jacobian(m,lam,bA,bB,c,lags,steps):
    N=len(lam);Z=[np.zeros(N) for _ in range(4)];names='FABC';J=np.zeros((len(lags),4*N),float)
    for r,lag in enumerate(lags):
...
def select_free_on_train(m,lam,bA,bB,c,train_lags,steps,P):
    ...
    return [dict(type=names[int(q//N)],index=int(q%N),flat=int(q),train_gradient=float(G[int(q//N)][int(q%N)]),
                 improve_sign=1.0 if G[int(q//N)][int(q%N)]>=0 else -1.0) for q in pick]
...
def greedy_nnls(A,target,ks=KS):
    ...
    chosen=[];coef=np.zeros(0);res=y.copy();out={};norms=np.linalg.norm(A,axis=0)+1e-30
    for t in range(1,max(ks)+1):
        corr=(A.T@res)/norms
        if chosen:corr[np.asarray(chosen,int)]=-np.inf
        j=int(np.argmax(corr))
        if not np.isfinite(corr[j]) or corr[j]<=0:break
        chosen.append(j)
        sol=lsq_linear(A[:,chosen],y,bounds=(0,np.inf),method='bvls',tol=1e-12,max_iter=5000)
        coef=np.maximum(sol.x,0.0);res=y-A[:,chosen]@coef
        if t in ks:out[str(t)]=dict(rel_residual=float(np.linalg.norm(res)/yn),selected=len(chosen),l1_coeff=float(np.sum(coef)),max_coeff=float(np.max(coef)),indices=[int(x) for x in chosen])
    ...
    for k in ks:out.setdefault(str(k),last.copy())
    return out


def one(m,train_lags,eval_lags,steps,P):
    wh,wv=ae.bond_weights(m,m.body);lam,bA,bB,c=modal_setup(m,wh,wv);selected=select_free_on_train(m,lam,bA,bB,c,train_lags,steps,P)
    edges,JL=local_feature_jacobian(m,wh,wv,eval_lags,steps);_,JF=free_feature_jacobian(m,lam,bA,bB,c,eval_lags,steps)
    rows=[]
    for q in selected:
        y=q['improve_sign']*JF[:,q['flat']];rows.append(dict(type=q['type'],index=q['index'],train_gradient=q['train_gradient'],target_norm=float(np.linalg.norm(y)),compiler=greedy_nnls(JL,y)))
...
def main():
    ...
    if a.selftest:
        A=np.eye(3);r=greedy_nnls(A,np.array([1.,2.,0.]),(1,2));assert r['2']['rel_residual']<1e-10;print('selftest ok');return
    ...
    for seed in range(a.seed_start,a.seed_start+a.seeds):
        ...
        print('seed',seed,{k:round(float(np.nanmedian([x['compiler'][str(k)]['rel_residual'] for x in r['selected']])),3) for k in KS},flush=True)
    sel=[x for r in rows for x in r['selected']];summary=dict(bodies=len(rows),selected_coordinates=len(sel),eval_features=len(ev),local_candidates=rows[0]['local_candidates'] if rows else 0)
    for k in KS:
        ...
        summary[str(k)]=dict(mean_rel_residual=float(np.nanmean(rr)),median_rel_residual=float(np.nanmedian(rr)),fraction_residual_lt_0_1=float(np.nanmean(rr<.1)),fraction_residual_lt_0_2=float(np.nanmean(rr<.2)),median_l1_coeff=float(np.nanmedian(cc)))
